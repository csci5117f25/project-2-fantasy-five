        ...
        <div class="col-lg-9 main-container">
          ...
          <div class="main-content">
            ...
            <div v-if="filteredOutfits?.length" class="row g-4">
              <div
                v-for="(outfit, outfitIndex) in filteredOutfits"
                :key="outfit.id"
              ...
              <div class="bg-light image-container">
                ...
                <div v-if="outfit.collages && outfit.collages.length" :id="'carousel-' + outfit.id + '-mobile'" class="carousel slide" data-bs-ride="carousel">
                  ...
                  <button
                    ...
                    class="carousel-control-prev"
                    type="button"
                    :data-bs-target="'#carousel-' + outfit.id + '-mobile'"
                    data-bs-slide="prev"
                    @click.stop
                  >
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
...
<script setup>
  ...
  const outfitsQuery = computed(() => {
    if (!currentUser.value) return query(collection(db, '_placeholder'))
    return query(collection(db, 'users', currentUser.value.uid, 'outfits'), orderBy('createdAt', 'desc'))
  })
  ...
  const filteredOutfits = computed(() => {
    ...
    return outfits.value.filter(outfit => {
      ...
      if (favorites?.includes('Favorites Only') && !outfit.favorite) return false
      return true
    })
  })
  ...
  const toggleFavorite = async (outfit) => {
    ...
    try {
      ...
      await updateDoc(outfitRef, { favorite: !outfit.favorite, updatedAt: serverTimestamp() })
    } catch (error) {
